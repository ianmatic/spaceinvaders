...
MyCamera& Simplex::MyCamera::operator=(MyCamera const& other)
{
	...
	{
		Release();
		SetPositionTargetAndUpward(other.cameraPos, other.cameraTarget, other.m_v3Above);
		MyCamera temp(other);
		Swap(temp);
	}
	return *this;
...
void Simplex::MyCamera::CalculateViewMatrix(void)
{
	...
	m_m4View = rotationMatrix * m_m4View;

	m_m4View = glm::lookAt(cameraPos, cameraPos + cameraFront, cameraUp); //position, target, upward
...
void MyCamera::MoveForward(float a_fDistance)
{
	...
	//update position
	cameraPos += cameraFront * a_fDistance;
...
void MyCamera::MoveVertical(float a_fDistance) {
	...
	vector3 cameraRight = glm::normalize(glm::cross(cameraUp, cameraDirection));
	...
	//update position
	cameraPos += glm::normalize(glm::cross(cameraFront, cameraRight)) * a_fDistance;
...
void MyCamera::MoveSideways(float a_fDistance)
{
	...
	//update position
	cameraPos += glm::normalize(glm::cross(cameraFront, cameraUp)) * a_fDistance;
...
void MyCamera::ChangePitchAndYaw(float xAngle, float yAngle)
{
		yaw += yAngle * 10;

	//Don't allow to look too far down or up
	if (abs(pitch + xAngle * 10) <= 90) {
		pitch += xAngle * 10;
	}
